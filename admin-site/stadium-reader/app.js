...
function renderContents(edition, articles) {
  ...
  $('#contents-partners').hidden = !edition.magazineAds?.length;
  $('#contents-partner-links').innerHTML = edition.magazineAds?.length ? `<details class="advertising-contents"><summary>Alle ${edition.magazineAds.length} Anzeigen im Heft</summary>${articles.map((article, index) => {
    if (!article.adSlots?.length) return '';
    const pageLink = issueLink(edition, `artikel/${article.id}`);
    const sponsors = article.adSlots.map(({ad}) => {
      const website = sponsorWebsiteUrl(ad.websiteUrl);
      const label = `Website von ${ad.name} öffnen (neuer Tab)`;
      return `<div class="partner-entry"><a class="partner-article-link" href="${pageLink}"><strong>${esc(ad.name)}</strong></a>${website ? `<a class="partner-website" href="${esc(website)}" target="_blank" rel="noopener noreferrer" aria-label="${esc(label)}" title="${esc(label)}"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M15 3h6v6M10 14 21 3M21 14v5a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5"/></svg></a>` : ''}</div>`;
    }).join('');
    return `<div class="contents-row partner-row"><div class="partner-names"><span>Anzeige</span>${sponsors}</div><a class="partner-page-link" href="${pageLink}" aria-label="Anzeigen auf Seite ${index + 2} ansehen">${pad(index + 2)}</a></div>`;
  }).join('')}</details>` : '';

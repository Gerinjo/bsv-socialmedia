-- Per-issue cover choices are frozen alongside the publication snapshot.
alter table public.editorial_issues add column cover_settings jsonb;
alter table public.editorial_issues add constraint editorial_cover_settings_object check (cover_settings is null or (jsonb_typeof(cover_settings)='object' and octet_length(cover_settings::text)<=65536));

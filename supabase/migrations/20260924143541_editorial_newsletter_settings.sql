-- Stored on the existing private, versioned editorial issue. No new grants.
alter table public.editorial_issues add column newsletter_settings jsonb;
alter table public.editorial_issues add constraint editorial_newsletter_settings_object check (
  newsletter_settings is null or (
    kind='newsletter' and jsonb_typeof(newsletter_settings)='object' and octet_length(newsletter_settings::text)<=16384
  )
);
...
create or replace function private.editorial_snapshot(target uuid) returns jsonb language sql security invoker set search_path='' as $$
  select jsonb_build_object('schemaVersion',1,'id',i.id,'version',i.version,'title',i.title,'kind',i.kind,'publishes_on',i.publishes_on,'newsletter_settings',i.newsletter_settings,

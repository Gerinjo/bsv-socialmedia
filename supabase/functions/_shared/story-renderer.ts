...
function multilineText(value: unknown, { x, y, width = 900, fontSize = 42, lineHeight = 52, maximumLines = 3, className = 'sans', fill = '#10251a', weight = 700, textAnchor }: { x: number; y: number; width?: number; fontSize?: number; lineHeight?: number; maximumLines?: number; className?: string; fill?: string; weight?: number; textAnchor?: 'start' | 'middle' | 'end' }): string {
...
function storyContentBlock(match: StoryInput, hasSponsors = false): string {
...
function lineupMatchup(match: StoryInput): { bsvTeam: unknown; opponent: unknown; matchType: string } {
  const bsvIsHome = teamCrestKey(match.homeTeam) === 'bsv';
  const bsvIsAway = teamCrestKey(match.awayTeam) === 'bsv';
  return {
    bsvTeam: bsvIsAway ? match.awayTeam : match.homeTeam,
    opponent: bsvIsAway ? match.homeTeam : match.awayTeam,
...
export function renderStorySvg({
  ...
  const values = {
    ...
    SPONSOR_LOGOS: sponsorLogoStrip(type, sponsorLogoDataUris),
  };

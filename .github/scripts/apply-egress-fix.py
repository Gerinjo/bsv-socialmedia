...
renderer_path = Path('supabase/functions/story-renderer/index.ts')
renderer = renderer_path.read_text()

old = "    const stamp = Date.now();\n    const renderedPages: Array<{ mediaUrl: string; storagePath: string }> = [];"
new = "    const jobSegment = safeSegment(body.jobId);\n    const previewFolder = `generated/${body.type}/${jobSegment}`;\n    const renderedPages: Array<{ mediaUrl: string; storagePath: string }> = [];"
if old not in renderer:
    raise SystemExit('Renderer marker for timestamp block not found')
renderer = renderer.replace(old, new, 1)

old = "      const storagePath = `generated/${body.type}/${safeSegment(body.jobId)}/${stamp}${suffix}.png`;"
new = "      const storagePath = `${previewFolder}/preview${suffix}.png`;"
if old not in renderer:
    raise SystemExit('Renderer storage-path marker not found')
renderer = renderer.replace(old, new, 1)

old = "          cacheControl: '3600',"
new = "          cacheControl: '604800',"
if old not in renderer:
    raise SystemExit('Renderer cache-control marker not found')
renderer = renderer.replace(old, new, 1)

old = ".createSignedUrl(storagePath, 60 * 60 * 24 * 7);"
new = ".createSignedUrl(storagePath, 60 * 60 * 24 * 30);"
if old not in renderer:
    raise SystemExit('Renderer signed-url marker not found')
renderer = renderer.replace(old, new, 1)

old = """      renderedPages.push({ mediaUrl: signed.signedUrl, storagePath });
    }

    const mediaUrls = renderedPages.map((page) => page.mediaUrl);"""
new = """      renderedPages.push({ mediaUrl: signed.signedUrl, storagePath });
    }
    ...
    const mediaUrls = renderedPages.map((page) => page.mediaUrl);"""
if old not in renderer:
    raise SystemExit('Renderer cleanup insertion marker not found')
renderer = renderer.replace(old, new, 1)
renderer_path.write_text(renderer)

api_path = Path('supabase/functions/social-media-admin-api/index.ts')
api = api_path.read_text()
start = api.index('async function freshPreviewUrls(admin: any, rows: any[]): Promise<any[]> {')
end = api.index("function cleanupStoragePaths(kind: 'game' | 'post' | 'story', record: any): string[] {", start)

replacement = r'''const signedUrlTtlSeconds = 60 * 60 * 24 * 30;
...
admin_path = Path('admin-site/admin-page.html')
admin = admin_path.read_text()
before = len(re.findall(r'<img\b', admin, flags=re.I))
admin = re.sub(r'<img\b(?![^>]*\bloading=)', '<img loading="lazy" decoding="async"', admin, flags=re.I)
after = admin.count('loading="lazy"')
if before == 0 or after == 0:
    raise SystemExit('No admin images were updated for lazy loading')
admin_path.write_text(admin)

for temporary in [
    Path('.github/workflows/apply-egress-fix.yml'),
    Path('.github/workflows/apply-egress-fix-pr.yml'),
    Path('.github/scripts/apply-egress-fix.py'),
]:
    if temporary.exists():
        temporary.unlink()
